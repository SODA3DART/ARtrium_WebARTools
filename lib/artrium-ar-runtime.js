...
(function () {
  ...
  function buildScene(config) {
    ...
    const mindSrc = encodeURI(config.marker.target).replace(/#/g, '%23');
    container.innerHTML = `
<a-scene
  mindar-image="imageTargetSrc: ${mindSrc}; uiLoading: no; uiScanning: no; uiError: no;"
  color-space="sRGB"
...
<div id="artrium-marker-overlay" class="artrium-marker-overlay" role="dialog" aria-modal="true" aria-labelledby="artrium-marker-title" hidden>
  ...
  function buildMobileHint(config) {
    if (!isMobileDevice()) return;

    if (document.getElementById('artrium-mobile-hint')) return;

    const marker = config.marker || {};
    document.body.insertAdjacentHTML(
      'beforeend',
      `
<div id="artrium-mobile-hint" role="status">
  <strong>マーカーの映し方</strong>
  「マーカーを表示」で出る画像と<strong>同じもの</strong>をカメラに映してください。<br>
  PC の絵・別の画像・ゲーム画面では認識されません。<br>
  印刷したポスター、または別のスマホでマーカー画像を表示するのが確実です。
  <span class="tap-close">タップで閉じる</span>
</div>`
    );
    document.getElementById('artrium-mobile-hint')?.addEventListener('click', (e) => {
      e.currentTarget.remove();
    });
  }

  function initMobileLayout() {
    document.documentElement.classList.add('artrium-ar-page');

    const applySize = () => {
      const h = window.innerHeight;
      const root = document.getElementById('artrium-scene-root');
      const scene = document.querySelector('#artrium-scene-root a-scene');
      if (root) {
    ...
    if (scene) {
      scene.addEventListener('loaded', applySize);
      scene.addEventListener('renderstart', applySize);
    }
